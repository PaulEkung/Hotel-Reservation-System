...
if(isset($_POST["submit"]))
{
    ...
    $sql = "SELECT stu.*, fe.*, ma.* FROM `students`stu, `payment-2`fe, `payment`ma WHERE stu.email = fe.email OR stu.email = ma.email AND stu.unique_id ='$id'";
    ...
    "<thead>" . 
    "<br>" .
    " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href='total-users.php' class='bi bi-arrow-left-circle offset-0 text-secondary fs-1'></a> " .
  ...
  {
    $search = $_POST["name"];
    $sql = "SELECT stu.*, fe.*, ma.* FROM `students`stu, `payment-2`fe, `payment`ma WHERE stu.email = fe.email OR stu.email = ma.email AND stu.fullnames = '$search'";
    $result = mysqli_query($connect, $sql);
    if($result == true){
...
print("<a href='delete.php?delete_id=$payId' class='bi bi-check-circle fs-2 text-success offset-4'></a>");
